...
public class GrpcService extends BotGrpc.BotImplBase {
    public BotenAnnaBot bot;
    private TimeTracker timeTracker = new TimeTracker();
    private Map<Integer, BotenAnnaBot> registeredBots = new HashMap<>();
    private ArrayBlockingQueue<BotenAnnaBot> botUpdateQueue;

    public GrpcService(ArrayBlockingQueue<BotenAnnaBot> botUpdateQueue) {
        ...
        try {
            ...
            synchronized (this) {
                if (!registeredBots.containsKey(playerIndex)) {
                    ...
                    BotenAnnaBot bot = new BotenAnnaBot(playerIndex, teamIndex, tree);
                    registeredBots.put(playerIndex, bot);
                }
            }

            // This is the bot that needs to think
            BotenAnnaBot bot = registeredBots.get(playerIndex);
